...
if ( isset( $_POST[ 'btn-signup' ] ) )
 {
    ...
    if ( $uname == '' )	 {
        ...
        if ( $sql )
        {
            ...
            echo '<script>alert("Registration successfully for login click on Login button");</script>';
            $icon = 'success';
            $class = 'success';
...
<h1><a href = 'index.php'><font style = 'font-size:25px; color:green'>Help Each Other</font></a></h1>
</div>
...
<li><a style = 'cursor:pointer' data-toggle = 'modal' data-target = '#signupModal'><i class = 'fa fa-user' ></i> Sign Up</a></li>
<li><a style = 'cursor:pointer' data-toggle = 'modal' data-target = '#myModal'><i class = 'fa fa-lock'></i> Log In</a></li>
</ul>
...
<!-- Announce -->
<div class = 'announce'>
We’ve over <strong>54 000</strong> Post offers for you!
</div>
...
<div class = 'modal-body'>
Select Your Photo: <input type = 'file' name = 'img' required/>
</div>
...
<?php
$sql = mysqli_query( $con, 'select * from posts join registration where posts.usr_id_p=registration.usr_id order by posts.status_time desc limit 6' );
while( $row = mysqli_fetch_array( $sql ) )
 {
    ...
    echo '<span><i class=""></i> </span><br><br>';
    echo '<span><i class=""></i><font style="color:blue">'.timeAgo( $time_ago ).'</font></span>';
    echo '</div>';
    // echo '';
    echo '<a data-toggle = "modal" data-target = "#myModal" class="btn btn-default"style="padding-left:25px;cursor:pointer;" >comment</a>';
    echo '<br/><br/>';
    echo '</li>';
...
<div class = 'copyrights'>©  Copyright 2023-24 by <a href = '#'>ME</a>. All Rights Reserved.</div>
</div>
</div>
</div>
